 ...
 *
 * 6c.6.1 wiring: parse config → construct sidecar client → build ToolDeps →
 ...
 * (predicate only at 6c.6.1; summariser glue at 6c.6.2) → register the
 * `agent_end` hook (mirror + save). 6c.7 reply_dispatch + lifecycle
 * (6c.8 / 6d) still deferred.
 */
...
const memgptPlugin = definePluginEntry({
  ...
  register(api: OpenClawPluginApi): void {
    ...
    api.logger.info(
      `openclaw-memgpt: 7 tools + before_prompt_build (prompt-section + flush-pressure) + agent_end hooks registered (namespace: ${config.namespace}, observability: ${config.observability})`,
    );
  },
});
